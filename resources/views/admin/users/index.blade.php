...
<style>
    ...
    .badge.user { background: rgba(16, 185, 129, 0.2); color: #86efac; }
...
@section('content')
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
        <h3 class="section-title" style="margin: 0;"><i class="fas fa-users"></i> Daftar Pengguna</h3>
        <a href="/users/create" class="btn-add"><i class="fas fa-user-plus"></i> Tambah Pengguna</a>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $user->class ?? '-' }}</td>
                    <td>
                        <a href="/users/{{ $user->id }}/edit" class="btn-action btn-edit"><i class="fas fa-edit"></i> Edit</a>
                        @if($user->id !== auth()->id())
                            <form action="/users/{{ $user->id }}" method="POST" style="display:inline;" onsubmit="return confirm('Yakin ingin menghapus pengguna ini?')">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="6" style="text-align: center; color: #a0aec0; padding: 40px;">
                        <i class="fas fa-users-slash" style="font-size: 32px; margin-bottom: 10px;"></i>
