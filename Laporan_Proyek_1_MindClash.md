...
<p align="center">
  <img src="https://raw.githubusercontent.com/laravel/art/master/logo-lockup/5%20SVG/2%20CMYK/1%20Full%20Color/laravel-logolockup-cmyk-red.svg" width="300" alt="ULBI Logo Placeholder"><br>
...
```mermaid
graph TD
    subgraph Browser ["Client-Side (User Browser)"]
        UI["UI (Blade + Tailwind CSS)"]
        JS["Client Logic (Timer & Carousel)"]
    end

    subgraph Server ["Server-Side (Laravel Monolith)"]
        Route["Routing (web.php)"]
        Mid["Middleware (Auth & Role)"]
        Ctrl["Controllers"]
    ...
    UI <--> HTTP["HTTP Requests / Responses"] <--> Route
    Route --> Mid
    Mid --> Ctrl
    Ctrl --> Model
    Model <--> DB
...
flowchart TD
    ...
    Login -- Belum --> HalamanLogin[Halaman Login] --> IsiKredensial[Input Email & Password] --> Autentikasi{Kredensial Valid?}
    Autentikasi -- Tidak --> IsiKredensial
    Autentikasi -- Ya --> Home[Halaman Utama / Dashboard Siswa]
    Login -- Ya --> Home
    Home --> PilihKategori[Pilih Kategori Kuis] --> DetailKuis[Tampil Detail & Batas Waktu]
    DetailKuis --> MulaiKuis[Klik Mulai Kuis] --> TampilSoal[Soal Pertama Ditampilkan & Timer Dimulai]
    TampilSoal --> PilihJawaban[Pilih Jawaban A, B, C, atau D]
    PilihJawaban --> Navigasi{Navigasi?}
    Navigasi -- Selanjutnya --> SoalBerikutnya[Tampilkan Soal Berikutnya] --> PilihJawaban
    Navigasi -- Sebelumnya --> SoalSebelumnya[Tampilkan Soal Sebelumnya] --> PilihJawaban
    Navigasi -- Selesai / Waktu Habis --> KirimJawaban[Kirim Jawaban ke Server]
    KirimJawaban --> HitungSkor[Server Hitung Skor & Simpan Hasil]
    HitungSkor --> TampilHasil[Tampilkan Halaman Hasil & Nilai]
    TampilHasil --> ReviewJawaban[Review Detail Jawaban Benar/Salah] --> End([Selesai])
```

#### **B. Workflow Manajemen Data Soal oleh Admin**
```mermaid
flowchart TD
    Start([Mulai]) --> LoginAdmin[Login Sebagai Admin] --> Dashboard[Dashboard Admin]
    Dashboard --> PilihMenu[Pilih Menu Manajemen Soal]
    PilihMenu --> Aksi{Pilih Aksi CRUD}
...
erDiagram
    ...
    QUESTIONS {
        ...
        text question_text
        varchar_255 option_a
        varchar_255 option_b
        varchar_255 option_c
        varchar_255 option_d
        char_1 correct_answer
        timestamp created_at
        timestamp updated_at
    }
...
![Mockup Halaman Kuis](report_images/quiz.png)
...
![Mockup Peringkat Kelas](report_images/leaderboard.png)
...
![Tampilan Halaman Kuis](report_images/quiz.png)
